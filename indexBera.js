...
const port = process.env.PORT_BERA || 3007;


//CONSTANTS
const providerUrl = process.env.PROVIDER_URL_BERA;
const poolfactory = process.env.CONTRACT_POOL_FACTORY_BERA;
const contract_memefactory= process.env.CONTRACT_MEME_FACTORY_BERA;
const contract_WETH = process.env.CONTRACT_WETH_BERA;
...
const getDbDataDePools = async () => {
  try {
    const query = `
      ...
      FROM data_pools_tbera
    `;
    
    const result = await dbConnection.query(query);

    // Aquí puedes trabajar con los resultados
    console.log("Datos de la tabla data_pools_tbera:", result.rows);
    ...
    console.error('Error al obtener los datos de data_pools_tbera:', error);
  }
};
...
const reservspooldatasave = async (pairpoolContract, token_contract) => {
  try {
    ...
    const query = `
      INSERT INTO tbera_${token_contract.slice(1)} (open, high, low, close, timestamp)
      VALUES ($1, $2, $3, $4, $5)
    `;
...
const checkAddingLiquidity = async (sponsor, token_contract) => {
  ...
  const tablepairAddress = "TBERA_"+token_contract.slice(1).toLowerCase();

  try {
    // Verificar si el pool_pair ya existe en la tabla
    const checkQuery = `
      SELECT 1 FROM data_pools_tbera WHERE token_contract = $1 LIMIT 1
    `;
    ...
    if (checkResult.rows.length === 0) {
      ...
      const insertQuery = `
        INSERT INTO data_pools_tbera (sponsor, token_contract, pool_pair)
        VALUES ($1, $2, $3)
      `;
